...
import CadastroRPA from '../../components/common/CreateRPAForm';
import { EditItemForm } from '../../components/common/EditItemForm';
import { editRPA, getUserRPAs } from '../../api/userApi';
...
export default function RPAPage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        const reqBody = {sortBy:userSortBy,
                         ...
                         query:searchQuery};
        try {
            const response = await getUserRPAs(reqBody);
    ...
    const handleEditSubmit = async (requestBody, itemId) => {
        try {
            ...
            await editRPA(requestBody, parametros);
            handleCloseDialog();
            fetchData();
        } catch (err) {
    ...
    return (
        <Box sx={{ p: 3 }}>
            ...
            >
                <Typography variant="h4" gutterBottom>Meus RPAs</Typography>
                <Box sx={{ display: 'flex', gap: 2, ml: 2 }}>
                    <CadastroRPA onDone={fetchData}/>
            ...
            <Paper sx={{ p: 2, mb: 3, mt: 3 }}>
                <Grid container spacing={2} alignItems="center">
                    ...
                    <OrderBy userSortBy={userSortBy} setUserSortBy={setUserSortBy}
                             sortableFields={CONFIGS.rpas.sortableFields} />
            ...
            <DataTable data={data} loading={loading} pageNumber={pageNumber} pageSize={pageSize}
                       setPageNumber={setPageNumber} pagination={pagination} columns={CONFIGS.rpas.columns}
                       deletable={false} onDelete={null} onEdit={handleEditClick} />
            ...
            <EditItemForm
                ...
                item={editingItem}
                dataType={'rpas'}
                editableFields={CONFIGS.rpas.editableFields}
                validationSchema={CONFIGS.rpas.validationSchema}
            />
